(() => {
  ...
  async function decodePhoto(file){
    if(/heic|heif/i.test(file.type)||/\.(heic|heif)$/i.test(file.name)){
      ...
      const converted=await heic2any({blob:file,toType:'image/jpeg',quality:.9}); file=Array.isArray(converted)?converted[0]:converted;
    }
  ...
  async function compressPhoto(file){
    const image=await decodePhoto(file), max=1800, ratio=Math.min(1,max/Math.max(image.width,image.height));
    ...
    const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',.86)); if(!blob) throw new Error('Não foi possível preparar a foto.'); return blob;
  }
  ...
  function renderFeatured(){const p=state.posts[0],img=$('#featuredImage'),ph=$('#featuredPlaceholder');if(!p){img.hidden=true;ph.hidden=false;$('#featuredAdventure').textContent='Compartilhe sua experiência';$('#featuredDescription').textContent='As novas publicações aparecem primeiro e também continuam na galeria abaixo.';$('#featuredDate').textContent='Galeria dos Trilheiros';return;}ph.hidden=true;img.hidden=true;img.src=`/api/photo?id=${encodeURIComponent(p.photoKey)}&v=${encodeURIComponent(p.id)}`;img.onload=()=>img.hidden=false;$('#featuredAdventure').textContent=p.title;$('#featuredDescription').textContent=p.description||'Uma nova memória compartilhada com os Trilheiros.';$('#featuredDate').textContent=formatDate(p.adventureDate||p.createdAt);}
  ...
  async function deletePost(p){if(!state.admin)return;if(!confirm(`Excluir “${p.title}”?`))return;try{const res=await fetch(`/api/posts?id=${encodeURIComponent(p.id)}`,{method:'DELETE',headers:{'x-admin-key':state.adminKey}});const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data.message||'Não foi possível excluir.');state.posts=state.posts.filter(x=>x.id!==p.id);renderFeatured();renderGallery();showToast('Publicação excluída.');}catch(e){showToast(e.message||'Erro ao excluir.');}}
