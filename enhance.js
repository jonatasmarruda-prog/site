(() => {
  ...
  const logoCss = getComputedStyle(document.documentElement).getPropertyValue('--tr-logo-data').trim();
  const LOGO_DATA = logoCss.replace(/^url\(["']?/, '').replace(/["']?\)$/, '');
  const startBtn = $('#startBtn');
  const installBtn = $('#installBtn');
  if (!startBtn || !installBtn) return;

  const logoSelectors = ['.brand-logo-mini','.hero-logo','.tracking-logo img','footer img'];
  logoSelectors.forEach(sel => $$(sel).forEach(img => { if (LOGO_DATA) img.src = LOGO_DATA; img.removeAttribute('crossorigin'); }));
  ...
  style.textContent = `
    ...
    .welcome-kicker b{display:block;font-size:9px;letter-spacing:.105em;color:#fff3cc}.welcome-kicker small{display:block;font-size:8px;color:#91a69e;margin-top:2px}
    .hero-title{font-size:clamp(38px,10vw,56px)!important;line-height:.94!important;margin-top:8px!important}.hero-copy{max-width:455px!important;font-size:12px!important}
    .install-card{margin-top:0!important;margin-bottom:12px!important;position:relative!important;overflow:hidden!important;border-color:rgba(241,198,111,.27)!important;background:linear-gradient(125deg,rgba(46,185,123,.14),rgba(241,198,111,.08))!important;display:flex!important}
    .install-card::before{content:"";position:absolute;top:0;left:-55%;width:36%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.09),transparent);transform:skewX(-18deg);animation:trShine 4.8s ease-in-out infinite}.install-icon{background:linear-gradient(135deg,rgba(241,198,111,.15),rgba(110,212,193,.10))!important}.install-card.installed{border-color:rgba(70,218,148,.28)!important;background:rgba(70,218,148,.08)!important}
    .tr-modal{position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.76);backdrop-filter:blur(11px);display:none;align-items:flex-end;justify-content:center;padding:14px}.tr-modal.show{display:flex}.tr-sheet{width:min(560px,100%);background:linear-gradient(160deg,#10372f,#061b17);border:1px solid rgba(241,197,107,.25);border-radius:28px;padding:18px;color:#f7f1df;box-shadow:0 35px 90px rgba(0,0,0,.65)}.tr-handle{width:44px;height:4px;border-radius:99px;background:#ffffff22;margin:0 auto 14px}.tr-icon{font-size:36px}.tr-sheet h3{font-size:22px;margin:8px 0 6px}.tr-sheet>p{color:#9fb0ab;font-size:12px;line-height:1.5;margin:0}.tr-steps{display:grid;gap:8px;margin:14px 0}.tr-step{display:flex;gap:10px;padding:11px;border-radius:14px;background:#ffffff09;border:1px solid #ffffff0d}.tr-num{width:27px;height:27px;border-radius:9px;display:grid;place-items:center;background:#f1c56b18;color:#f1c56b;font-size:10px;font-weight:900;flex:none}.tr-step b{display:block;font-size:11px}.tr-step span{display:block;color:#879b95;font-size:9px;margin-top:2px;line-height:1.4}.tr-close{width:100%;height:50px;border:1px solid #ffffff12;border-radius:15px;background:#14382f;color:#fff;font-weight:900}
  ...
  const modal = document.createElement('div'); modal.className = 'tr-modal'; modal.innerHTML = `<div class="tr-sheet"><div class="tr-handle"></div><div class="tr-icon">📲</div><h3></h3><p></p><div class="tr-steps"></div><button class="tr-close">ENTENDI</button></div>`; document.body.appendChild(modal);
  ...
  const embeddedLogo=new Image();if(LOGO_DATA)embeddedLogo.src=LOGO_DATA;const generateBtn=$('#generateBtn');if(generateBtn)generateBtn.addEventListener('click',()=>{setTimeout(()=>{const canvas=$('#artCanvas');if(!canvas||!embeddedLogo.complete)return;const ctx=canvas.getContext('2d'),w=canvas.width,h=canvas.height,size=Math.round(Math.min(w,h)*.18),pad=Math.round(w*.045);ctx.save();ctx.shadowColor='rgba(0,0,0,.48)';ctx.shadowBlur=24;ctx.drawImage(embeddedLogo,pad,pad,size,size);ctx.restore();},120);},false);
})();
